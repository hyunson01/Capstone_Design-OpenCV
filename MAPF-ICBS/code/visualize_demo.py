...
class Animation:
    def __init__(self, my_map, starts, goals, paths, offsets=None):
        ...
        for i, goal in enumerate(self.goals):
            goal_patch = Rectangle((goal[0] - 0.25, goal[1] - 0.25), 0.5, 0.5,
                                   facecolor=Colors[i % len(Colors)],
                                   edgecolor='black', alpha=0.5)
            goal_patch.set_visible(False)
            self.patches.append(goal_patch)
            self.goal_patches.append(goal_patch)
